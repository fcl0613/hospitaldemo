...
/**
 * Servlet implementation class ReleaseAnnouncement
 */
@WebServlet("/manage/releaseannouncement")
public class ReleaseAnnouncement extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int res = announcementServiceImpl.releaseAnnouncement(Integer.valueOf(idStr));
		PrintWriter out = response.getWriter();
		if (res > 0) {
			out.write("<script>");
			out.write("alert('公告发布成功');");
			out.write("location.href='selectannouncement';");
			out.write("</script>");
		}else {
			out.write("<script>");
			out.write("alert('公告发布失败');");
			out.write("location.href='selectannouncement';");
			out.write("</script>");
		}
		out.close();
